...
def print_recommendation(recommendation_df, songs_df):
    for msd_id in recommendation_df['msd_id']:
        print(msd_id)
        print(songs_df.loc[songs_df['song_id'] == msd_id, 'title'], 'by',
              songs_df.loc[songs_df['song_id'] == msd_id, 'artist_name'])
    # for index, row in recommendation_df.iterrows():
    #     print(row['artist_name'], row['title'])
